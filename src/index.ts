...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        if (url.pathname === '/api/chat') {
            try {
                const { message, parentMode, sessionId } = await request.json();

                // Get or create agent instance for this session
                const agentId = env.SOCIAL_SKILLS_AGENT.idFromName(sessionId || 'default-session');
        ...
        if (url.pathname === '/api/exercise') {
            try {
                const { exerciseType, sessionId } = await request.json();
                const agentId = env.SOCIAL_SKILLS_AGENT.idFromName(sessionId || 'default-session');
                const agent = env.SOCIAL_SKILLS_AGENT.get(agentId);
